...
- Require a newly created default administrator to replace its temporary
  password after the first successful login.

Important:
Existing administrators are not forced to reset again during every seed
run. Use ``scripts/require_default_password_reset.py`` when an existing
default administrator must be required to change its password again.
